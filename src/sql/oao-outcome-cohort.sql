...
WITH oao_concept_set (concept_id) as
(
  SELECT DISTINCT I.concept_id 
  FROM
  (
    SELECT concept_id FROM @vocabulary_database_schema.concept 
      WHERE concept_id IN (606805, 439223, 437158, 4230779, 4032799, 438120, 438130, 4099935)
    UNION ALL
    SELECT c.concept_id 
    FROM @vocabulary_database_schema.concept c
    JOIN `@vocabulary_database_schema.concept_ancestor ca 
      ON c.concept_id = ca.descendant_concept_id
    AND ca.ancestor_concept_id IN (606805, 439223, 437158, 4230779, 4032799, 438120, 438130, 4099935)
