SELECT subject_id AS row_id,
       ...
       COUNT(vo.visit_concept_id) AS covariate_value
...
AND vo.visit_start_date <= DATEADD(d, @end_day, c.cohort_start_date)
{@start_day != -9999} ? {AND vo.visit_start_date >= DATEADD(d, @start_day, c.cohort_start_date)}
